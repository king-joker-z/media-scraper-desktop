...
| `src/main/core/`               | 基础设施：scanner（扫描+指纹）、task-center、settings、probe（ffprobe+缓存）、frames（截帧/场景切分）、fs-ops（唯一文件写入口）、image（sharp）、file-hash、op-log、ffmpeg-pool、process-registry、lru-cache、media-path、task-report |
| `src/main/modules/`            | 业务能力：clean / merge / comic（scan+epub+pdf+merge）/ rename(execute+ai) / poster / nfo / dedupe / undo                                                                                                                             |
| `src/shared/`                  | `types.ts`（类型唯一来源）、`rename-rules.mjs`、`merge-rules.mjs`、`comic-rules.mjs`（纯函数规则，双端复用）                                                                                                                          |
| `src/renderer/src/pages/`      | 视频模块 8 个页面 + `ComicMergePage` / `ComicLibraryPage`                                                                                                                                                                             |
