...
Electron 39 + React 19 + TypeScript + electron-vite（Vite 7）；`ffmpeg-static` / `ffprobe-static` 随包分发；`sharp` 图片转码；`fflate` 构建 EPUB；`pdf-lib` 构建 PDF。
...
- **增量追更**：已合并后仅增加完整新章节时，只处理新章节并追加至已有 EPUB/PDF；已有章节增加/删除图片或调整顺序、切换输出格式、希望重新应用图片优化策略时，需勾选「全量重建」。
